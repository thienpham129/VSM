	...
	SecurityFilterChain applicationSecurity(HttpSecurity http) throws Exception {
		...
		http.csrf(AbstractHttpConfigurer::disable) // Disable CSRF if it’s not needed
				...
				.authorizeHttpRequests(
						registry -> registry.requestMatchers("/", "/public/**", "/auth/**", "/uploads/**").permitAll()
								.requestMatchers("/user").hasRole("USER").requestMatchers("/admin/**").hasRole("ADMIN")
								.requestMatchers("/", "/public/**", "/auth/**", "/assets/**").permitAll()
								.requestMatchers("/admin/**").hasRole("ADMIN").requestMatchers("/driver/**")
								.hasAnyRole("DRIVER", "ADMIN").requestMatchers("/api/v1/**").permitAll().anyRequest()
								.authenticated())
